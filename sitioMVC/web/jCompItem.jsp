...
String mensaje = "";
ResultSet compItem;
if(request.getSession().getAttribute("compItem") != null){
    compItem = (ResultSet)request.getSession().getAttribute("compItem");
    if(compItem.getString(1).equals("-1")){
        mensaje += "<script languague='javascript'>"
                + "alert('El ID del item no existe, verificar.');"
                + "</script>";
    }
    if(compItem.getString(1).equals("2")){
        mensaje += "<script languague='javascript'>"
                + "alert('El número de control del alumno a prestar no existe, verificar.');"
                + "</script>";
    }
    if(compItem.getString(1).equals("-1")){
        mensaje += "<script languague='javascript'>"
                + "alert('El item ya esta prestado, verificar.');"
                + "</script>";
    }
    if(compItem.getString(1).equals("0")){
        mensaje += "<script languague='javascript'>"
                + "alert('El prestamo del item fue realizado éxitosamente.');"
                + "</script>";
    }
...
<body>
  ...
  <div class="container-fluid">
    ...
    <section id="body">
        <div class="container">
            <div>
                <h1 class="title">Compartir item</h1>
            </div>
            <form action="" method="POST" class="formInput" id="formInput">
                ...
                <input type="submit" value="Prestar" onclick="verificar()">
                    ...
                    if(receptor.value === ""){
                        ...
                        if(item.value === ""){
                            ...
                            if(confirm("¿Estás seguro de continuar?")){
                                document.forms[0].action = "srvCompItem";
