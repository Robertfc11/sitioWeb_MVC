...
@WebServlet(name = "srvDevItems", urlPatterns = {"/srvDevItems"})
public class srvDevItems extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            ...
            if(!lugar.equals("") && !fecha.equals("") && !receptor.equals("") && 
                ...
                ResultSet rs;
                rs = objUsuario.spDevItemsPrestados(lugar, fecha, receptor, item);
                rs.next();
                request.getSession().setAttribute("devItem", rs);
                request.getRequestDispatcher("jDevolvItem.jsp").forward(request, response);
            }else{
                request.getRequestDispatcher("jDevolvItem.jsp").forward(request, response);
            }
        }catch(SQLException ex){
            Logger.getLogger(srvDevItems.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
